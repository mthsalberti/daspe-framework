                            ...
                            <div class="card-body">
                                ...
                                <form method="POST" action="{{ route('password.request') }}" aria-label="{{ __('Reset Password') }}">
                                    ...
                                    <div class="md-form mt-5">
                                        ...
                                        <input type="password" id="password" class="form-control mb-1" name="password" value="{{ old('password') }}" required>
                                        <label for="password" class="label-active-force">Senha</label>
                                        <div class="position-absolute pl-5">
                                            @if($errors->has('password')) <small class="red-text white px-1">{{$errors->first('password')}}</small> @endif
                                        </div>
                                    </div>

                                    <div class="md-form mt-5">
                                        <i class="fa fa-lock prefix white-text"></i>
                                        <input type="password" id="password_confirmation" class="form-control mb-1" name="password_confirmation" value="{{ old('password_confirmation') }}" required>
                                        <label for="password_confirmation" class="label-active-force">Confirmação de senha</label>
                                        <div class="position-absolute pl-5">
                                            @if($errors->has('password_confirmation')) <small class="red-text white px-1">{{$errors->first('password_confirmation')}}</small> @endif
                                        </div>
                                    </div>

                                    <div class="text-center">
                                        <button type="submit" class="btn btn-block bg-main btn-lg p-0 py-2">Alterar senha</button>
                                    </div>
                                </form>
